...
-- build the schema", not a spec of it â€” asserting every column here
-- would just be the migrations restated in a second place, drifting.
DO $$
BEGIN
  -- The core tables, from 001.
  IF to_regclass('public.messages') IS NULL THEN
    RAISE EXCEPTION 'public.messages is missing â€” migrations did not apply';
  END IF;
  IF to_regclass('public.whatsapp_config') IS NULL THEN
    RAISE EXCEPTION 'public.whatsapp_config is missing â€” migrations did not apply';
  END IF;
  ...
  IF to_regclass('storage.buckets') IS NULL THEN
    RAISE EXCEPTION
      'storage.buckets is missing â€” the storage schema was not available when the bucket migrations ran';
  END IF;
  ...
  IF to_regclass('public.accounts') IS NULL THEN
    RAISE EXCEPTION 'public.accounts is missing â€” migration 017 did not apply';
  END IF;
  ...
  IF to_regclass('public.idx_contacts_account_wa_user_id') IS NULL THEN
    RAISE EXCEPTION
      'idx_contacts_account_wa_user_id is missing â€” migration 040 did not apply';
  END IF;
  ...
  -- first call â€” plpgsql resolves names at execution, not CREATE, so a
  ...
  IF pg_get_functiondef(
    ...
    RAISE EXCEPTION
      'create_broadcast_with_recipients still has the ambiguous RETURNING â€” migration 041 did not apply';
  END IF;

  -- The failure-reason columns (042) are only ever written by the
  -- status webhook, which uses an untyped update â€” a missing column
